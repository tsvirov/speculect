...
progress bar). All the numbers it prints are the real output of speculect's
library functions against the same fixture files — the animation is
cosmetic, the data isn't.
...
`examples/wow_demo.gif`, embedded at the top of `README.md`, is a separately
rendered storyboard of this same walkthrough (same models, same verdicts,
same bench numbers) — built for a README that can't run Python. It isn't
regenerated by the `--svg` flag above; see the repo history if you need to
reproduce it.
