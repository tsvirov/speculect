...
![speculect scan/pair/bench walkthrough](examples/wow_demo.svg)

*Real output, not a mockup — generated by `examples/wow_demo.py --svg`, see
below.*
...
Real captured output: [examples/README.md](examples/README.md). The
animated/SVG version above is `examples/wow_demo.py` — `pip install -e
".[demo]"` then `python examples/wow_demo.py` for the live animated version
in your own terminal, or `--svg out.svg` to regenerate the static image.
